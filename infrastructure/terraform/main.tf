...
resource "oci_core_route_table" "public_rt" {
  compartment_id = var.compartment_ocid
  vcn_id         = oci_core_vcn.k3s_vcn.id
  display_name   = "atlaspay-public-rt"
...
resource "oci_core_subnet" "k3s_subnet" {
  ...
  compartment_id    = var.compartment_ocid
  vcn_id            = oci_core_vcn.k3s_vcn.id
  display_name      = "atlaspay-k3s-subnet"
...
resource "oci_core_instance" "k3s_node" {
  ...
  compartment_id      = var.compartment_ocid
  display_name        = "atlaspay-k3s-node"
  shape               = var.instance_shape
  ...
  metadata = {
    ...
    user_data           = base64encode(file("${path.module}/install_k3s.sh"))
  }
}
